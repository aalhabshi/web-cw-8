// ------------------------------ 1️⃣ السؤال الأول ------------------------------
//     - driving اسمها function عرف دالة او
//     - تأخذ عمر شخص (رقم) واذا كان اكبر من 18 تطبع
//          🚘😍 مبروك سوف تحصل على الليسن
//     - واذا كان اقل من 18 يطبع
//          😢 للاسف عمرك لا يسمح
//     console.log ، if ملاحظة: سوف تستخدم

// 👇 هنا function عرف الدالة او
let ahmed = 21
    
let abdullah = 17
        
function driving(x){
    if(x >= 18){
       console.log("🚘😍 مبروك سوف تحصل على الليسن")
    }else{
        console.log("😢 للاسف عمرك لا يسمح")
    };
}

// لا تنسى أن تنادي الدالة



// ------------------------------ 2️⃣ السؤال الثاني ------------------------------
// 2️⃣ السؤال الثاني
//     - meals واجعل اسمها array عرف مصفوفة او
//     - 🥘 احفظ فيها مجموعة من طعامك المفضل
//     - console.log لطباعة قائمة بطعامك المفضل بإستخدام for loop استخدم

// 👇 هنا array عرف المصفوفة او
let meals = ['pasta, pizza, shrimp']

for (let i = 0; i < meals.length; i++) {
console.log(meals[i]) 
}

// --------------------------------- ✨ بونص ------------------------------------
// بحيث عند مناداة الدالة تطبع القائمة myFood بإسم function عرف دالة او
// for loop عرف مصفوفة أخرى تحتوي على أفلامك المفضلة وقم بطباعة الأفلام بإستخدام

// --------------------------------- 🔥 بونص ------------------------------------
// while loop اطبع احد المصفوفات بإستخدام

